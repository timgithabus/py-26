...
class FireArrow(arcade.Sprite):
    def __init__(self, x, y, target_x, target_y):
        super().__init__()
        sprite = make_sprite_from_candidates(
            [f"images/fire_arrow.jpg", f"images/fire_arrow.png", "images/fire_arrow.jpg", "images/fire_arrow.png"],
            scale=0.08,
            fallback_w=14,
            fallback_h=14,
...
class IceBall(arcade.Sprite):
    def __init__(self, x, y, target_x, target_y):
        super().__init__()
        sprite = make_sprite_from_candidates(
            [f"images/ice_ball.jpg", f"images/ice_ball.png", f"images/ice_ball.jpg", f"images/ice_ball.png"],
            scale=0.22,
            fallback_w=18,
            fallback_h=18,
            fallback_color=arcade.color.BLUE,
...
class WitchDoktorBullet(arcade.Sprite):
    def __init__(self, x, y, target_x, target_y):
        ...
        sprite = make_sprite_from_candidates(
            [
                f"images/witch_doctor_bullet.png",
                f"images/Witch_doctor_bullet.png",
                f"images/witch_doctor_bullet.jpg",
                f"images/Witch_doctor_bullet.jpg",
            ],
            scale=0.1,
            fallback_w=16,
...
class OrkBossBullet(arcade.Sprite):
    def __init__(self, x, y, target_x, target_y):
        ...
        sprite = make_sprite_from_candidates(
            [
                f"images/ork_boss_bullet.png",
                f"images/ork_boss_bullet.jpg",
                f"images/Witch_doctor_bullet.png",
                f"images/witch_doctor_bullet.png",
            ],
            scale=0.15,
...
class EnemiesPudge(EnemyBase):
    def __init__(self, x, y, target, wave):
        super().__init__(
            [f"images/enemy_pudge.png", f"images/enemy_pudge.jpg", "images/enemy_pudge.png"],
            scale=0.05,
            x=x,
            y=y,
            target=target,
        ...
        # Здоровье зависит от волны
        self.max_hp = float(40 + wave * 12)
        self.hp = float(self.max_hp)
        self.base_speed = 1.7 + min(1.6, wave * 0.01)
        self.damage = int(random.randint(5, 10) * (1.0 + wave * 0.03))
        self.shooting = False
...
class WitchDoktor(EnemyBase):
    def __init__(self, x, y, target, wave):
        super().__init__(
            [f"images/witch_doktor.jpg", f"images/witch_doktor.png", f"images/witch_doktor.jpg",
             f"images/witch_doktor.png"],
            scale=0.08,
            x=x,
            y=y,
            target=target,
        ...
        # Здоровье зависит от волны
        self.max_hp = float(WITCH_DOKTOR_MAX_HP + wave * 6)
        self.hp = float(self.max_hp)
...
class FireArchers(EnemyBase):
    def __init__(self, x, y, target, wave):
        super().__init__(
            [f"images/fire_archer.jpg", f"images/fire_archer.png", f"images/fire_archer.jpg",
             f"images/fire_archer.png"],
            scale=0.06,
            x=x,
            y=y,
            target=target,
        ...
        # Здоровье зависит от волны
        self.max_hp = float(60 + wave * 9)
        self.hp = float(self.max_hp)
        self.base_speed = 1.6 + min(1.2, wave * 0.006)
        self.damage = int(random.randint(5, 10) * (1.0 + wave * 0.03))
...
class BossDragon(EnemyBase):
    def __init__(self, x, y, target, wave):
        super().__init__(
            [
                f"images/blue_dragon.png",
                f"images/ice_dragon.jpg",
                f"images/ice_dragon.png",
                f"images/blue_dragon.jpg",
                f"images/ice_dragon.png",
                f"images/blue_dragon.png",
            ],
            scale=0.35,
        ...
        # Здоровье зависит от волны
        self.max_hp = float(BOSS_MAX_HP + wave * 60)
        self.hp = float(self.max_hp)
        self.base_speed = 2.6
...
class OrkBoss(EnemyBase):
    def __init__(self, x, y, target, wave):
        super().__init__(
            [f"images/ork_boss.jpg", f"images/ork_boss.png", "images/ork_boss.jpg", "images/ork_boss.png"],
            scale=0.7,
            x=x,
            y=y,
            target=target,
        ...
        # Здоровье зависит от волны
        self.max_hp = float(ORC_BOSS_MAX_HP + wave * 45)
        self.hp = float(self.max_hp)
        self.base_speed = float(ORC_BOSS_SPEED)
        self.damage = int(ORC_BOSS_DAMAGE + wave * 2)
        self.attack_cooldown = 0.0
...
class Start_menu(arcade.View):
    def __init__(self):
        ...
        if self.map is None:
            self.map = load_texture_safe("images/map.png")
        self.speed = 5
        self.selected_hero = 0
        self.hero_textures = []
        for i in range(4):
            tex = load_texture_safe(f"images/hero{i + 1}.png")
            if tex is None:
    ...
    def on_draw(self):
        ...
        if self.map is not None:
            arcade.draw_texture_rect(rect.x, rect.y, rect.width, rect.height, self.map)
        else:
...
class MyGame(arcade.View):
    def __init__(self, selected_hero=0):
        ...
        self.hero_sprite = Hero([f"images/{hero_image}", f"{hero_image}"], scale=0.25)
        ...
        self.skill_textures = {}
        skill_images = {
            1: ["images/revival.jpg", "images/revival.jpg"],
            2: ["images/heal.jpg", "images/heal.jpg"],
            3: ["images/crits.jpg", "images/crits.jpg"],
            4: ["images/lighting.jpg", "images/lighting.jpg"],
            5: ["images/stink.jpg", "images/stink.jpg"],
            6: ["images/armor.jpg", "images/armor.jpg"],
            7: ["images/invise.jpg", "images/invise.jpg"],
            8: ["images/golem.jpg", "images/golem.jpg"],
            9: ["images/мулти_удар.png", "images/мулти_удар.png"]
        }

        for skill_id, paths in skill_images.items():
            tex = None
            for path in paths:
                tex = load_texture_safe(path)
                if tex is not None:
                    break
            self.skill_textures[skill_id] = tex

        # Иконки для выбора скиллов (большие)
        self.skill_select_textures = {}
        for skill_id, paths in skill_images.items():
            tex = None
            for path in paths:
                tex = load_texture_safe(path)
                if tex is not None:
                    break
            self.skill_select_textures[skill_id] = tex
        ...
        self.skill9_enabled = False  # Вонючка теперь переключаемая
        self.skill9_toggle_key_pressed = False
    ...
    def draw_hud(self):
        ...
        # Статус вонючки
        if 9 in self.skills:
            status = "ВКЛ" if self.skill9_enabled else "ВЫКЛ"
            color = arcade.color.GREEN if self.skill9_enabled else arcade.color.RED
            arcade.draw_text(
                f"Вонючка: {status} (T)",
                self.view_width - 200,
                self.view_height - 100,
    ...
    def draw_skills_bar(self):
        ...
        for skill_id in skills_sorted:
            ...
            # Рисуем иконку скилла
            tex = self.skill_textures.get(skill_id)
            if tex is not None:
                # Рисуем иконку
                scale = icon / max(tex.width, tex.height)
                scaled_width = tex.width * scale
                scaled_height = tex.height * scale

                # Центрируем иконку
                icon_x = x + (icon - scaled_width) / 2
                icon_y = y + (icon - scaled_height) / 2

                arcade.draw_texture_rectangle(
                    icon_x + scaled_width / 2,
                    icon_y + scaled_height / 2,
                    scaled_width,
                    scaled_height,
                    tex
                )
            ...
            # Номер скилла в углу
            arcade.draw_text(str(skill_id), x + 6, y + icon - 14, arcade.color.BLACK, 12, bold=True)
            ...
            if skill_id == 1:
                ...
                # Для вонючки показываем статус включения
                if self.skill9_enabled:
                    arcade.draw_text("ON", x + icon // 2, y - 14, arcade.color.GREEN, 10,
    ...
    def on_key_press(self, key, modifiers):
        ...
        # Переключение вонючки клавишей T
        if key == arcade.key.T and 9 in self.skills:
            self.skill9_enabled = not self.skill9_enabled
            self.skill9_toggle_key_pressed = True
            return
    ...
    def maybe_proc_skill9(self):
        if 9 not in self.skills or not self.skill9_enabled:
            return
    ...
    def activate_skill(self, num):
        ...
        if num == 2:
            ...
            # Вонючка теперь переключается клавишей T, а не активируется
            pass
    ...
    def try_summon_golem(self, x, y):
        ...
        golem = make_sprite_from_candidates(
            [f"images/golem.png", "images/golem.jpg", "golem.png"],
            scale=0.35,
            fallback_w=90,
            fallback_h=90,
            fallback_color=arcade.color.ORANGE,
    ...
    def spawn_enemy(self):
        ...
        w = self.wave
        if w < 3:
            enemy = EnemiesPudge(x, y, self.hero_sprite, self.wave)  # Передаем волну вместо уровня
        elif w < 6:
            if r < 0.7:
                enemy = EnemiesPudge(x, y, self.hero_sprite, self.wave)  # Передаем волну вместо уровня
            else:
                enemy = FireArchers(x, y, self.hero_sprite, self.wave)  # Передаем волну вместо уровня
        else:
            if r < 0.6:
                enemy = EnemiesPudge(x, y, self.hero_sprite, self.wave)  # Передаем волну вместо уровня
            elif r < 0.85:
                ...
                enemy = WitchDoktor(x, y, self.hero_sprite, self.wave)  # Передаем волну вместо уровня
    ...
    def spawn_bullet(self, x, y, target_x, target_y, damage):
        bullet = make_sprite_from_candidates(
            [f"images/bull.png", "images/bull.jpg", "bull.png"],
            scale=0.12,
            fallback_w=10,
            fallback_h=10,
    ...
    def draw_skill_select(self):
        ...
        for r in range(rows):
            for c in range(cols):
                ...
                if skill_id in self.skills:
                    ...
                    # Иконка скилла
                    tex = self.skill_select_textures.get(skill_id)
                    if tex is not None:
                        scale = size / max(tex.width, tex.height) * 0.8
                        scaled_width = tex.width * scale
                        scaled_height = tex.height * scale
                        icon_x = x + (size - scaled_width) / 2
                        icon_y = y + (size - scaled_height) / 2
                ...
                if tex is not None:
                    ...
                    # Номер скилла
                    arcade.draw_text(str(skill_id), x + size - 10, y + size - 10, arcade.color.BLACK, 14,
        ...
        skill_descriptions = {
            1: "Вторая жизнь - срабатывает при смерти",
            2: "Вампиризм - лечение при убийстве врагов",
            3: "Критический удар - шанс 35% на доп. урон",
            4: "Молния - АОЕ урон с замедлением",
            5: "Аура - постоянный урон врагам вокруг",
            6: "Сопротивление - уменьшение получаемого урона",
            7: "Невидимость - враги игнорируют вас",
            8: "Голем - призывает помощника",
            9: "Мульти удар - шанс сбросить КД других скиллов (T)"
        }

        # Показываем описание при наведении
        mouse_x, mouse_y = self.window._mouse_x, self.window._mouse_y
        for skill_id, bx, by, bw, bh in self.skill_buttons:
            if bx <= mouse_x <= bx + bw and by <= mouse_y <= by + bh:
                if skill_id in skill_descriptions:
                    desc_x = mouse_x + 10
                    desc_y = mouse_y + 10
                    arcade.draw_lbwh_rectangle_filled(desc_x - 5, desc_y - 5, 200, 30, arcade.color.WHITE)
                    arcade.draw_lbwh_rectangle_outline(desc_x - 5, desc_y - 5, 200, 30, arcade.color.BLACK)
                    arcade.draw_text(
    ...
    def spawn_boss(self):
        ...
        boss = BossDragon(x, y, self.hero_sprite, self.wave)  # Передаем волну вместо уровня
